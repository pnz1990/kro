...
// goSeededInt is the reference FNV-1a implementation for verifying random.seededInt.
func goSeededInt(seed string, max int64) int64 {
...
func TestRandomSeededInt(t *testing.T) {
	env, err := cel.NewEnv(Random())
	require.NoError(t, err)
	...
	tests := []struct {
		...
		{
			name: "basic roll",
			expr: "random.seededInt('test-seed', 20)",
			want: goSeededInt("test-seed", 20),
		},
		{
			name: "different seed different result",
			expr: "random.seededInt('other-seed', 20)",
			want: goSeededInt("other-seed", 20),
		},
		{
			name: "mod 100",
			expr: "random.seededInt('uid-shield', 100)",
			want: goSeededInt("uid-shield", 100),
		},
		{
			name: "mod 2 (coin flip)",
			expr: "random.seededInt('flip', 2)",
			want: goSeededInt("flip", 2),
		},
		{
			name: "mod 36",
			expr: "random.seededInt('dungeon-m0-rar', 36)",
			want: goSeededInt("dungeon-m0-rar", 36),
		},
		{
			name:    "max zero",
			expr:    "random.seededInt('seed', 0)",
			wantErr: true,
			errMsg:  "max must be positive",
		},
		{
			name:    "max negative",
			expr:    "random.seededInt('seed', -1)",
			wantErr: true,
			errMsg:  "max must be positive",
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err != nil {
					...
					assert.Contains(t, out.Value(), tt.errMsg)
				}
				return
			}
			require.NoError(t, err)
			...
			result, ok := out.Value().(int64)
			require.True(t, ok, "expected int64, got %T", out.Value())
			assert.Equal(t, tt.want, result)

			// Verify determinism
			out2, _, err := prg.Eval(map[string]interface{}{})
			require.NoError(t, err)
			assert.Equal(t, result, out2.Value().(int64), "seededInt must be deterministic")
		})
	}
}
